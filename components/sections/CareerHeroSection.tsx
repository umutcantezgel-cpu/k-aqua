...
export function CareerHeroSection() {
  ...
  return (
    <section className="relative w-full min-h-[100svh] h-auto py-32 lg:py-0 flex flex-col justify-center items-start bg-slate-50 pt-24 pb-16 overflow-hidden">
      ...
      <div className="max-w-[1440px] mx-auto px-6 sm:px-8 lg:px-12 relative z-10 w-full flex flex-col">
        <motion.div
          ...
          <motion.div variants={itemVariants} className="w-full min-w-0 max-w-full overflow-hidden mb-12">
            <p className="text-[clamp(1.125rem,2.5vw,1.5rem)] text-slate-600 font-light leading-relaxed max-w-2xl  w-full min-w-0 max-w-full overflow-hidden">
              {t('Hero.subtitle')}
            </p>
          </motion.div>
          ...
          <motion.div variants={itemVariants} className="w-full min-w-0 max-w-full overflow-hidden">
            <a 
              href="#open-roles"className="inline-flex items-center justify-center bg-white/60 backdrop-blur-xl border border-slate-200/50 shadow-[0_8px_30px_rgb(0,0,0,0.06)] text-slate-900 hover:bg-white hover:scale-[1.02] active:scale-[0.98] transition-all px-8 py-4 rounded-full text-base sm:text-lg font-medium ring-1 ring-black/[0.02]">
              {t('Hero.cta')}
            </a>
          </motion.div>
        </motion.div>
