...
export default function ServiceHeroSection() {
  ...
  return (
    <section className="relative w-full min-h-[100svh] h-auto py-32 lg:py-0 flex flex-col items-center justify-center overflow-hidden bg-[#fafafa]">
      ...
      <div className="absolute inset-0 w-full h-full overflow-hidden pointer-events-none flex items-center justify-center">
        ...
        <div className="absolute bottom-0 w-full h-72 md:h-[50vh] bg-gradient-to-t from-[#fafafa] to-transparent z-0"/>
      </div>
      ...
      <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-12 relative z-10 flex flex-col items-center text-center w-full min-w-0 max-w-full justify-center flex-1 py-32">
        <motion.div 
          ...
          initial="hidden"animate="visible"className="w-full min-w-0 max-w-5xl flex flex-col items-center">
          ...
          <motion.div variants={itemVariants} className="w-full min-w-0 max-w-full overflow-hidden flex justify-center">
            <h1 
              className="w-full min-w-0 max-w-full overflow-hidden text-[clamp(1.75rem,6vw,4.5rem)] leading-[1.05] tracking-tight  text-slate-900 break-words hyphens-auto"dangerouslySetInnerHTML={{ __html: t('title') }}
            />
          </motion.div>
          
          <motion.div variants={itemVariants} className="w-full min-w-0 max-w-full overflow-hidden flex justify-center mt-6 sm:mt-8">
            <p className="w-full min-w-0 max-w-[36ch] text-lg sm:text-xl md:text-[clamp(1.125rem,2vw,1.5rem)] text-slate-500 leading-relaxed  font-light">
              {t('subtitle')}
            </p>
          </motion.div>
